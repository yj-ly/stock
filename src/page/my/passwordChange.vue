...
<script>
  ...
  import {forgetPwdChange} from "../../api/user";
  ...
  export default {
    ...
    data(){
        return {
            ...
            passowrdBtn:true,  //修改密码的按钮不能重复点击
    ...
    methods:{
      ...
      changePassword() {
        ...
        if(this.phoneError && this.codeError && this.newPassword && this.rePassword && this.passowrdBtn){
          this.passowrdBtn = false;
          this.btnSure = true;
          setTimeout(() => {
            forgetPwdChange(this.changeModel).then(res => {
              if(res.data.status == 0){
                this.showScuess = true;
                this.errorMessage = res.data.msg;
                this.$refs.message.showModel = true;
                this.btnSure = false;
                this.passowrdBtn = true;
                setTimeout(() => {
